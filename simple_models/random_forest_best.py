...
class RandomForestPredictor:
    def __init__(self):
        ...
        self.artifact_dir = os.path.join(os.path.dirname(__file__), 'artifacts', 'random_forest_best')

    def save_artifacts(self):
        ensure_dir(self.artifact_dir)
        if self.model is None:
            raise RuntimeError('Nothing to save')
        save_joblib(self.model, os.path.join(self.artifact_dir, 'random_forest.joblib'))

    def load_artifacts(self):
        model = load_joblib(os.path.join(self.artifact_dir, 'random_forest.joblib'))
        if model is None:
            return False
        self.model = model
    ...
    def update_csv(self, csv_file='models_comparison_RES.csv'):
        ...
        col_name = f"RANDOM_FOREST_ORIGINAL"
        err_name = f"RANDOM_FOREST_ORIGINAL_%ERROR"
