...
class SimpleXGBoostPredictor:
    def __init__(self):
        ...
        # XGBoost Params (Matching your script)
        self.params = {
            'eval_metric': 'rmse',
            'eta': 0.1,
            'max_depth': 4,
        ...
        # artifact directory stored next to the module so loading works from other CWDs
        self.artifact_dir = os.path.join(os.path.dirname(__file__), 'artifacts', 'xgboost_best_original')

    def save_artifacts(self):
        ensure_dir(self.artifact_dir)
        meta = {'n_splits': self.N_SPLITS, 'params': self.params, 'num_boost_round': self.num_boost_round}
    ...
    def train(self, data_file='original_dataset.csv'):
        """Loads data and trains the K-Fold ensemble."""
        print(f"Loading data from {data_file}...")
        df = pd.read_csv(data_file)

        # Preprocessing (Just node adjustment)
        work_df = df.copy()
        work_df['NUM_NODES'] = work_df['NUM_NODES'] - 1

        X = work_df[self.INPUT_METRICS].values
        y = work_df[self.TARGET_METRIC].values
        ...
        print(f"Starting training on {len(df)} rows...")
        ...
        self.is_trained = True
        print("Simple XGBoost Training complete.")
        try:
            self.save_artifacts()
    ...
    def update_csv(self, csv_file='models_comparison_RES.csv'):
        ...
        # Save columns
        col_name = "XGBOOST_ORIGINAL"
        err_name = "XGBOOST_ORIGINAL_%ERROR"
...
if __name__ == "__main__":
    ...
    # 1. Train (or load existing artifacts)
