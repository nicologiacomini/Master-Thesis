...
class ResidualNN(nn.Module):
    def __init__(self, architecture, activation):
        ...
        self.layer_1 = nn.Linear(2, architecture[0])
...
def load_ffnn_model():
    model_ffnn = ResidualNN([64, 64], nn.Sigmoid())
...
def run_ffnn(n_workers, msize, bsize):
    ffnn_model, ffnn_scaler_X, ffnn_scaler_y = load_ffnn_model()
    ...
    X_scaled_ffnn = ffnn_scaler_X.transform(X_input)
    X_tensor_ffnn = torch.tensor(X_scaled_ffnn, dtype=torch.float32)
    ...
    ffnn_model.eval()
    with torch.no_grad():
        y_pred_scaled_ffnn = ffnn_model(X_tensor_ffnn).numpy()
    y_residual_pred_ffnn = ffnn_scaler_y.inverse_transform(y_pred_scaled_ffnn).flatten()[0]
    predicted_time_ffnn = usl_time + y_residual_pred_ffnn
    return float(predicted_time_ffnn)
